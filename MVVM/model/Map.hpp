...
public:
    ...
    BaseWall* GetWhichBlock(double x, double y)
	{
		int m, n;
    ...
    bool IsBlockWall(double x, double y)
	{
		MapUnit* t = GetWhichMapUnit(x, y);
		if (t->GetBlockState(x, y) == Base::wall|| t->GetBlockState(x, y) == Base::door)return true;
		return false;
    }
    MapUnit* GetWhichMapUnit(double x, double y)
	{
    ...
    MapUnit* GetWhichMapUnit(double x, double y,int &node)
	{
		node = -1;
		for (int i = 0;i < AllMapUnit.size();i++)
		{
